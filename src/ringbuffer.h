...
/// @tparam kMaxCapacity maximum possible buffer capacity. 
///
...
class RingBuffer
{
  ...
  private:
    ...
    /// Translates relative index in the buffer into absolute index in underlying array
    SizeType GetArrayElementIndex(const SizeType relative_index) const;

    /// Retrieves element from underlying array while checking for bounds
